...
if (!tourn) {
  console.error(`Tournament '${TOURNAMENT_ID}' not found. Available: ${TOURNAMENT_CONFIGS.map(t => t.id).join(', ')}`);
  process.exit(1);
}
    ...
    if (!QUIET) {
      ...
      if (state.tables) {
        const tables = state.tables
          .map(t => `T${t.tableNumber}(${t.gameState?.players?.filter(p => !p.folded && p.chips > 0).length ?? 0}p)`)
          .join(' ');
...
function verifyGlobalChips() {
  ...
  if (director.activeBots.size > 0 && director.tables.size > 0 && inTables + inPots === 0) {
    const msg = `GLOBAL chip conservation violated: ${director.activeBots.size} active bots but 0 chips in tables/pots`;
    errors.push(msg);
    logger.error('simulate', msg, { inTables, inPots, activeBots: director.activeBots.size });
  }
}
...
director.start().catch(e => {
  logger.critical('simulate', 'Tournament director crashed', {
    tournamentId: TOURNAMENT_ID,
    totalHands,
...
function printResults(results, tourn) {
  ...
  for (const r of results) {
    ...
    const medal  = pos.trim() === '1' ? '🏆' : pos.trim() === '2' ? '🥈' : pos.trim() === '3' ? '🥉' : '  ';
    console.log(`  ${pos}  ${name}  ${owner}  ${level.padEnd(7)}  ${payout} ${medal}`);
  }
  console.log('');
}
...
process.on('unhandledRejection', (reason) => {
  logger.critical('simulate', 'Unhandled rejection', {}, reason instanceof Error ? reason : new Error(String(reason)));
