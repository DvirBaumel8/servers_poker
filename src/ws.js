/**
 * ws.js — Zero-dependency WebSocket server
 *
...
class WsSocket extends EventEmitter {
  ...
  send(data) {
    ...
    try { this.socket.write(buildFrame(typeof data === 'string' ? data : JSON.stringify(data))); }
    catch (_) {}
  }
  ...
  close() {
    if (this.readyState !== 1) return;
    this.readyState = 2;
    try { this.socket.write(buildCloseFrame()); } catch (_) {}
    this.socket.end();
  }
}
...
function createWsServer(httpServer) {
  ...
  httpServer.on('upgrade', (req, socket, head) => {
    ...
    // Subscribe helper exposed on ws
    ws.subscribe = (tid) => {
  ...
  emitter.broadcast = (tableId, data) => {
  ...
  emitter.clientCount = (tableId) => {
    return clients.get(tableId)?.size || 0;
  };

  emitter.totalClients = () => {
    let n = 0;
