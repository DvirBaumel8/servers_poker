...
// ============================================================================
// CONDITION GROUPS (AND/OR logic for recursive predicate trees)
// ============================================================================

export type LogicalOperator = "AND" | "OR";

/**
 * A logical grouping of conditions using AND or OR operator.
 * May be nested to arbitrary depth (validator enforces MAX_CONDITION_DEPTH).
 *
 * Discriminant: ConditionGroup has a `rules` array; plain Condition has `category`+`field`.
 */
...
export interface Rule {
  ...
  priority: number;
  /** Condition nodes evaluated with AND logic at the top level. Nodes may be ConditionGroups for nested AND/OR. */
...
// ============================================================================
// HYDRATED STRATEGY (compiled in-memory representation for fast evaluation)
// ============================================================================

/**
 * Rules pre-sorted by priority and pre-filtered (disabled rules removed).
 * Eliminates O(n log n) sort + filter on every action call.
 */
export interface HydratedStreetRules {
  readonly preflop: ReadonlyArray<Rule>;
  readonly flop: ReadonlyArray<Rule>;
  readonly turn: ReadonlyArray<Rule>;
  readonly river: ReadonlyArray<Rule>;
}

/**
 * Range chart converted from a plain object to a Map for O(1) hand-notation lookups.
 */
export interface HydratedRangeChart {
  /** Dense 169-byte lookup table indexed by hand notation index. 0=fallthrough, 1=fold, 2=call, 3=raise */
  readonly lut: Uint8Array;
}

/**
 * One fully-resolved position context: personality already merged with any override,
 * rules compiled, range chart mapped.
 */
export interface HydratedPosition {
  readonly personality: Readonly<Personality>;
  readonly rules: HydratedStreetRules;
  readonly rangeChart: HydratedRangeChart | null;
}

/**
 * The compiled form of a BotStrategy.
 *
 * Built once at game-start via `hydrateStrategy()` and reused for every action
 * in that game. Pass to `evaluateHydrated()` instead of the raw BotStrategy to
 * avoid repeating sort/filter/merge on every decision.
 */
export interface HydratedStrategy {
  readonly tier: StrategyTier;
  /** Base strategy (used when no position override applies). */
