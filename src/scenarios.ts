...
function assertContains(str, substr, msg) {
  if (!String(str).includes(substr)) throw new Error(msg || `Expected "${str}" to contain "${substr}"`);
}
...
function enter(tournament_id, bot_id) {
  return db.createEntry({ tournament_id, bot_id, chips_at_entry: 5000, entry_type: 'initial' });
}
...
async function runTournament(tournamentId, botIds, opts = {}) {
  return new Promise((resolve, reject) => {
    ...
    for (const botId of botIds) {
      db.createEntry({ tournament_id: tournamentId, bot_id: botId, chips_at_entry: 5000, entry_type: 'initial' });
    }
...
test('Registering a bot creates an entry with initial type', () => {
  ...
  const entry = db.createEntry({ tournament_id: cfg.id, bot_id: b.id, chips_at_entry: 5000, entry_type: 'initial' });
...
test('Registering the same active bot twice is rejected at app layer', () => {
  ...
  const b = makeBot(u.id);
  db.createEntry({ tournament_id: cfg.id, bot_id: b.id, chips_at_entry: 5000, entry_type: 'initial' });
  const entries = db.getEntries(cfg.id);
  // App-layer check: find active entry (finish_position IS NULL)
  const activeEntry = entries.find(e => e.bot_id === b.id && e.finish_position === null);
  ...
  assert(activeEntry.finish_position === null, 'Active entry has no finish_position');
  // The route returns 409 when activeEntry is found — simulate that check
  const wouldReject = !!entries.find(e => e.bot_id === b.id && e.finish_position === null);
...
test('Busted bot can rebuy (entry bust then new entry)', () => {
  ...
  const rebuy = db.createEntry({ tournament_id: cfg.id, bot_id: b.id, chips_at_entry: 5000, entry_type: 'rebuy' });
...
test('Registration blocked when tournament finished', () => {
  ...
  assert(tourn.status === 'finished', 'Should reject registration on finished tournament');
});
...
test('Registration blocked when tournament full', () => {
  ...
  const entries = db.getEntries(cfg.id).filter(e => e.finish_position === null);
...
testAsync('Late entry joins running tournament during valid window', async () => {
  ...
  const { results } = await runTournament(cfg.id, botIds.slice(0, 2), {
    onDirector(director) {
      ...
      director._onHandComplete = async function(tableDbId, results) {
        if (director.currentLevel <= cfg.late_reg_ends_level && !lateJoined && director.handsThisLevel >= 1) {
          lateJoined = true;
          // Create a DB entry for the late bot, then add via director
          db.createEntry({ tournament_id: cfg.id, bot_id: bots[2].id, chips_at_entry: 5000, entry_type: 'initial' });
          try {
            director.addLateEntry({ botId: bots[2].id, name: bots[2].name, endpoint: bots[2].endpoint });
          } catch(e) {
...
testAsync('Late entry rejected past late_reg_ends_level', async () => {
  ...
  assertContains(rejectionError, 'Late registration closed', `Expected "Late registration closed", got: "${rejectionError}"`);
});
...
testAsync('Tournament completes with crasher bots in the field', async () => {
  ...
  const { results } = await new Promise((resolve, reject) => {
    ...
    director._onHandComplete = async function(tableDbId, r) {
      try { await origOHC(tableDbId, r); }
      catch(e) { errors++; }
    };

    for (const bot of bots) {
      db.createEntry({ tournament_id: cfg.id, bot_id: bot.id, chips_at_entry: 5000, entry_type: 'initial' });
    }
...
testAsync('Bot strikes are recorded in bot_events after repeated failures', async () => {
  // Use a bot that always crashes to trigger strike system
  const cfg = makeTournament({ min_players: 2 });
  const u1 = makeUser('strike1'); const b1 = makeBot(u1.id, 'smart');
  const u2 = makeUser('strike2'); const b2 = makeBot(u2.id, 'caller');
...
testAsync('2-bot heads-up tournament completes correctly', async () => {
  ...
  const u1 = makeUser('hu1'); const b1 = makeBot(u1.id, 'smart');
  const u2 = makeUser('hu2'); const b2 = makeBot(u2.id, 'caller');
  ...
  assertEqual(results[0].payout, cfg.buy_in * 2, 'Winner takes all in 2-player');
});

testAsync('9-bot single-table freezeout completes', async () => {
  const cfg = makeTournament({ min_players: 2, max_players: 9, rebuys_allowed: false });
  const personalities = ['smart', 'caller', 'maniac', 'random', 'folder', 'allin', 'smart', 'caller', 'maniac'];
  const bots = personalities.map((p, i) => {
    const u = makeUser(`st9_${i}`); return makeBot(u.id, p);
  });

  const { results } = await runTournament(cfg.id, bots.map(b => b.id));

  assertEqual(results.length, 9, '9 entrants should have results');
  assert(results.every(r => r.finish_position !== null), 'All should have finish positions');
  // Payouts for top 3 (50/30/20)
  const totalPayout = results.reduce((s, r) => s + r.payout, 0);
  assertEqual(totalPayout, cfg.buy_in * 9, 'Total payouts should equal prize pool');
});

testAsync('18-bot multi-table tournament reaches final table and completes', async () => {
  const cfg = makeTournament({ min_players: 2, max_players: 18 });
  const personalities = ['smart','caller','maniac','random','folder','allin'];
  const bots = Array.from({ length: 18 }, (_, i) => {
    const p = personalities[i % personalities.length];
    const u = makeUser(`mt18_${i}`); return makeBot(u.id, p);
  });
  ...
  const totalPayout = results.reduce((s, r) => s + r.payout, 0);
  assertEqual(totalPayout, cfg.buy_in * 18, 'Total payouts should equal prize pool');
});
...
testAsync('Payouts sum to exactly the prize pool — no chips created or lost', async () => {
  const cfg = makeTournament({ buy_in: 250, min_players: 2, max_players: 9 });
  const bots = Array.from({ length: 9 }, (_, i) => {
    const u = makeUser(`pay${i}`); return makeBot(u.id, 'caller');
  });
  const expectedPool = 250 * 9;
  const { results } = await runTournament(cfg.id, bots.map(b => b.id));
  const totalPayout = results.reduce((s, r) => s + r.payout, 0);
  assertEqual(totalPayout, expectedPool, `Payouts ${totalPayout} should equal prize pool ${expectedPool}`);
});

testAsync('2-player payout: winner takes 100%', async () => {
  const cfg = makeTournament({ buy_in: 500, min_players: 2, max_players: 9 });
  const u1 = makeUser('p2a'); const b1 = makeBot(u1.id, 'smart');
  const u2 = makeUser('p2b'); const b2 = makeBot(u2.id, 'folder');
  const { results } = await runTournament(cfg.id, [b1.id, b2.id]);
...
testAsync('6-player payout: top 2 paid at 65/35', async () => {
  ...
  const bots = Array.from({ length: 6 }, (_, i) => {
    const u = makeUser(`p6_${i}`); return makeBot(u.id, i % 2 === 0 ? 'smart' : 'caller');
  });
  const pool = 100 * 6;
  const { results } = await runTournament(cfg.id, bots.map(b => b.id));
  ...
  // Check 65/35 split
  assert(paidResults[0].payout === Math.floor(pool * 0.65) || paidResults[0].payout === Math.ceil(pool * 0.65),
...
testAsync('Blind levels advance every HANDS_PER_LEVEL hands', async () => {
  const cfg = makeTournament({ min_players: 2, max_players: 9 });
  const u1 = makeUser('bl1'); const b1 = makeBot(u1.id, 'caller');
  const u2 = makeUser('bl2'); const b2 = makeBot(u2.id, 'caller');
    ...
    const director = new TournamentDirector({
      ...
      onStateUpdate: (id, state) => {
        ...
        if (!last || last.level !== state.level) {
          levelChanges.push({ level: state.level, hands: state.handsThisLevel });
        }
      },
      onFinished: (id, r) => resolve({ director, results: r }),
    });

    db.createEntry({ tournament_id: cfg.id, bot_id: b1.id, chips_at_entry: 5000, entry_type: 'initial' });
    db.createEntry({ tournament_id: cfg.id, bot_id: b2.id, chips_at_entry: 5000, entry_type: 'initial' });
    db.updateTournamentStatus(cfg.id, 'running');
  ...
  assert(levelChanges.length >= 2, `Should have advanced at least 2 levels, got ${levelChanges.length}`);
...
testAsync('Blinds on game instances update when level advances', async () => {
  const cfg = makeTournament({ min_players: 2, max_players: 9, late_reg_ends_level: 4 });
  const u1 = makeUser('badv1'); const b1 = makeBot(u1.id, 'caller');
  const u2 = makeUser('badv2'); const b2 = makeBot(u2.id, 'caller');
      ...
      onStateUpdate: (id, state) => {
        if (state.blinds) {
          ...
          if (!last || last.small !== state.blinds.small) {
            blindSnapshots.push({ level: state.level, small: state.blinds.small, big: state.blinds.big });
          }
        }
      },
  ...
  if (blindSnapshots.length >= 2) {
    assert(blindSnapshots[1].small > blindSnapshots[0].small, 'Blinds should increase at level advance');
  }
});
...
testAsync('All hands are persisted to DB after tournament', async () => {
  const cfg = makeTournament({ min_players: 2, max_players: 9 });
  const u1 = makeUser('di1'); const b1 = makeBot(u1.id, 'smart');
  const u2 = makeUser('di2'); const b2 = makeBot(u2.id, 'caller');
  ...
  for (const hand of hands) {
    assert(hand.big_blind > 0, `Hand ${hand.hand_number} should have big_blind recorded`);
    assert(hand.small_blind > 0, `Hand ${hand.hand_number} should have small_blind recorded`);
  }
});

testAsync('Actions are persisted including antes and blinds', async () => {
  const cfg = makeTournament({ min_players: 2, max_players: 9 });
  const u1 = makeUser('act1'); const b1 = makeBot(u1.id, 'smart');
  const u2 = makeUser('act2'); const b2 = makeBot(u2.id, 'caller');
  ...
  assert(antes.length > 0, `Should have ante actions, found: ${actions.map(a=>a.type).join(',')}`);

  // Should have blind actions
  const blinds = actions.filter(a => a.type === 'blind');
  assert(blinds.length >= 2, `Should have at least 2 blind actions, got ${blinds.length}`);

  // Every action should have pot_before and chips_before
  for (const action of actions.filter(a => a.type !== 'ante' && a.type !== 'blind')) {
    assert(action.chips_before >= 0, `Action ${action.id} should have chips_before`);
  }
});

testAsync('bot_stats updated after tournament', async () => {
  const cfg = makeTournament({ min_players: 2, max_players: 9 });
  const u1 = makeUser('bs1'); const b1 = makeBot(u1.id, 'smart');
  const u2 = makeUser('bs2'); const b2 = makeBot(u2.id, 'caller');
  ...
  assert(stats1.total_hands > 0, `Bot1 should have hands recorded, got ${stats1.total_hands}`);
  assert(stats2.total_hands > 0, `Bot2 should have hands recorded, got ${stats2.total_hands}`);
...
testAsync('Seat history recorded on table moves', async () => {
  const cfg = makeTournament({ min_players: 2, max_players: 18 });
  const bots = Array.from({ length: 18 }, (_, i) => {
    const u = makeUser(`sh${i}`); return makeBot(u.id, 'caller');
  });

  await runTournament(cfg.id, bots.map(b => b.id));
  ...
  assert(totalHistory > 0, `Should have seat history records, got ${totalHistory}`);
});
...
testAsync('Tournament with all-in bots completes (side pot stress)', async () => {
  const cfg = makeTournament({ min_players: 2, max_players: 9 });
  const bots = Array.from({ length: 6 }, (_, i) => {
  ...
  const totalPayout = results.reduce((s, r) => s + r.payout, 0);
  assertEqual(totalPayout, cfg.buy_in * 6, 'Prize pool must be fully distributed');
});

testAsync('Tournament with folder bots completes in reasonable time', async () => {
...
testAsync('Bot with 0 chips sits out and does not cause division by zero', async () => {
  // Maniac + allin should create 0-chip situations frequently
  const cfg = makeTournament({ min_players: 2, max_players: 9 });
  const bots = Array.from({ length: 4 }, (_, i) => {
    const p = i % 2 === 0 ? 'allin' : 'maniac';
    const u = makeUser(`zero${i}`); return makeBot(u.id, p);
  });
  const { results } = await runTournament(cfg.id, bots.map(b => b.id));
  assert(results.some(r => r.finish_position === 1), 'Should have a winner');
});
