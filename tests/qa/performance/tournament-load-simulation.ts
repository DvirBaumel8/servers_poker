  ...
  private async startTournament(index: number): Promise<void> {
    ...
    try {
      ...
      for (let i = 0; i < this.config.playersPerTournament; i++) {
        ...
        const strategy = PERSONALITY_STRATEGIES[personality];

        instance.botRecords.push({ botId, strategy });
        ...
        await this.dataSource!.query(
          ...
          [botId, this.userId, `Bot_${index}_${i}`, JSON.stringify(strategy)],
        );
